...
class STL10(Dataset):
    ...
    def __init__(self, root=MyPath.db_root_dir('stl-10'),
                 ...
                 download=False,number_of_samples=0,number_of_augmentations=1):
                 
        super(STL10, self).__init__()
        self.root = root
        self.transform = transform
        self.split = verify_str_arg(split, "split", self.splits)
        self.folds = self._verify_folds(folds) # works only with folds = None
        ...
        #3  split |-->  load data part to self.data, self.labels  (now load the picked numpy arrays)
        if self.split == 'train':
            self.data, self.labels = self.__loadfile(
                self.train_list[0][0], self.train_list[1][0]) # 'train_X.bin' , 'train_y.bin'
    ...
    def __getitem__(self, index):
        ...
        # return a PIL Image
        img = Image.fromarray(np.transpose(img, (1, 2, 0))) # pil.image.image
        img_size = img.size

        if self.transform is not None:
            img = self.transform(img)
        
        # out = {'image': img, 'target': target, 'meta': {'im_size': img_size, 'index': index % self.number_of_samples , 'class_name': class_name}}
    ...
    def __loadfile(self, data_file, labels_file=None):
        ...
        if labels_file:
            ...
            if self.number_of_samples > 0: 
                amount = min(self.number_of_samples,len(labels))
                labels = labels[0:amount]
                
                
            #labels = np.tile(labels,self.number_of_augmentations)
        ...
        with open(path_to_data, 'rb') as f:
            ...
            if self.number_of_samples > 0: 
                amount = min(self.number_of_samples,len(images))
                everything = everything[0:27648*amount]
                #everything = np.tile(everything,self.number_of_augmentations)
                images = np.reshape(everything, (-1, 3, 96, 96))
