...
def main():
    ...
    for k in list(state_dict.keys()): # <file>moco_v2_800ep_pretrain.pth.tar >(torch.load)> moco_state >(moco_state['state_dict'])> state_dict
        # Copy backbone weights; map: {all}state_dict[module.encoder_q{}] --> new_state_dict[module.backbone{x}] 
    ...
    model.load_state_dict(new_state_dict) # Load the [model] configuration from the state_dict
