...
def get_criterion(p):
    ...
    elif p['criterion'] == 'clPcl':
        from losses.losses import PclCldLoss
        criterion = PclCldLoss()
...
def get_model(p, pretrain_path=None):
    ...
    elif p['backbone'] == 'mnet':
        from models.mnet import mnist_model
        backbone = mnist_model()        
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        dataset = MNIST(split=split, transform=transform, download=True)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    if p['val_db_name'] == 'cifar-10':
        ...
        from data.mnist import MNIST
        dataset = MNIST(split='test', transform=transform, download=True)
...
def get_instance_model(p, backbone):
    from models.models import ContrastiveModel
    from models.clPcl_model import MoCo
    core_model_q = ContrastiveModel(backbone,head='linear',features_dim=p['model_kwargs']['features_dim'])
    return MoCo(encoder_model=core_model_q, dim=p['model_kwargs']['features_dim'])
    
    
def get_group_model(p, backbone):
    from models.models import ContrastiveModel
    return ContrastiveModel(backbone,head='linear',features_dim=p['model_kwargs']['features_dim'])
    
def get_clustering(p):
    return p['clustering']
